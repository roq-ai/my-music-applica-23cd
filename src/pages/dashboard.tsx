...
import { getViewers } from 'apiSdk/viewers';
...
function DashboardPage() {
  ...
  const viewerStat = useSWR<PaginatedInterface<ViewerInterface>>(
    () => `/viewers`,
    () => getViewers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Viewers',
      result: viewerStat?.data?.totalCount,
      isLoading: viewerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
